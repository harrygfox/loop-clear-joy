...
const SubmitModal = ({ isOpen, onClose, onSubmit, invoice, mode }: SubmitModalProps) => {
  ...
  const businessName = mode === 'sent' ? invoice.to : invoice.from;
  const actionLabel = mode === 'sent' ? 'submit' : 'approve';

  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-background rounded-xl p-6 max-w-sm w-full animate-scale-in">
        <div className="text-center mb-6">
          <Send size={48} className="text-primary mx-auto mb-3" />
          <h3 className="text-lg font-bold text-foreground mb-2">
            {mode === 'sent' ? 'Submit Invoice' : 'Approve Invoice'}
          </h3>
          <p className="text-sm text-muted-foreground">
            {mode === 'sent' 
              ? `Submit invoice to ${businessName}` 
              : `Approve invoice from ${businessName}`
            }
          </p>
          ...
          <label className="flex items-start space-x-3 cursor-pointer p-3 rounded-lg border border-border hover:bg-muted/30 transition-colors">
            ...
            <div>
              <p className="font-medium text-foreground">
                {actionLabel.charAt(0).toUpperCase() + actionLabel.slice(1)} this invoice once
              </p>
              <p className="text-xs text-muted-foreground">
                Handle this specific invoice only
              </p>
            </div>
          </label>
            ...
            <div className="flex items-start space-x-2">
              <div className="flex-1">
                <p className="font-medium text-foreground flex items-center space-x-1">
                  <span>Auto-{actionLabel} all future invoices from {businessName}</span>
                  <Zap size={14} className="text-warning" />
                </p>
                <p className="text-xs text-muted-foreground">
                  Create a clearing rule for this business
                </p>
              </div>
            </div>
          </label>
